...
import { useInView, useUserPrefs } from "@wethegit/react-hooks";
    ...
    <div
      ...
      {...(width &&
        height && {
          style: {
            "--aspect-ratio": `${
              parseFloat((height / width).toFixed(4)) * 100
            }%`,
          },
        })}
    >
      {prefersReducedMotion && renderReducedMotionFallback() ? (
        ...
        <>
          {descriptionID && (
            <p
              id={descriptionID}
              className="autoplay-video-util-visually-hidden"
            >
              {description}
            </p>
          )}
...
AutoplayVideo.propTypes = {
  ...
  lazyLoadRootMargin: PropTypes.string,
